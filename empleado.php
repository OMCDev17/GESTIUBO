...
<body class="bg-background-light dark:bg-background-dark min-h-screen text-slate-900 dark:text-slate-100">
                ...
                <div class="w-full max-w-[860px] flex flex-col gap-6">
                        ...
                        <div class="flex flex-col gap-8">
                            ...
                            <section>
                                <h3 class="text-primary text-sm font-bold uppercase tracking-wider mb-6 flex items-center gap-2">
                                    ...
                                    <span id="secAcademico">Origen AcadÃƒÂ©mico</span>
                                </h3>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-5">
                                        <p id="lblInstitucion" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">InstituciÃƒÂ³n</p>
                                        <p class="mt-2 text-base font-semibold text-slate-900 dark:text-slate-100"><?= $safe('institucion') ?></p>
                                    </div>
                                    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-5">
                                        <p id="lblPais" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">PaÃƒÂ­s</p>
                            ...
                            <section>
                                <h3 class="text-primary text-sm font-bold uppercase tracking-wider mb-6 flex items-center gap-2">
                                    ...
                                    <span id="secIncorporacion">Detalles de la IncorporaciÃƒÂ³n</span>
                                </h3>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-5">
                                        <p id="lblFin" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Fecha de FinalizaciÃƒÂ³n</p>
                                    ...
                                    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-5">
                                        ...
                                        <p class="mt-2 text-base font-semibold text-slate-900 dark:text-slate-100">
                                            <?php if ($hasPendingRequest && !$hasActiveStay) : ?>
                                                <?= htmlspecialchars(($pendingRequest['group_name'] ?? '-') . ' - Pendiente de aprobaciÃƒÂ³n / Pending approval', ENT_QUOTES, 'UTF-8') ?>
                                            <?php else : ?>
                                                <?= $safe('group_name') ?>
                                            <?php endif; ?>
                                        </p>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 p-8 mt-6">
                        ...
                        <?php if (!$hasActiveStay && !$hasPendingRequest && empty($stayHistory)) : ?>
                            <p class="text-sm text-slate-600 dark:text-slate-300">No hay estancias registradas aÃƒÂºn / No internships registered yet.</p>
                        <?php else : ?>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <?php if ($hasPendingRequest) : ?>
                                    <article class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-5">
                                        ...
                                        <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100"><?= htmlspecialchars($pendingRequest['institucion'] ?? '-', ENT_QUOTES, 'UTF-8') ?> Ã‚Â· <?= htmlspecialchars($pendingRequest['pais'] ?? '-', ENT_QUOTES, 'UTF-8') ?></p>
                                        ...
                                        <p class="mt-3 inline-flex rounded-full bg-amber-200 dark:bg-amber-900 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-amber-900 dark:text-amber-100">Pendiente de aprobaciÃƒÂ³n / Pending approval</p>
                                    </article>
                                <?php endif; ?>
                                <?php if ($hasActiveStay) : ?>
                                    <article class="rounded-xl border border-primary/40 bg-primary/5 dark:bg-primary/10 p-5">
                                        ...
                                        <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100"><?= htmlspecialchars($activeStay['institucion'] ?? '-', ENT_QUOTES, 'UTF-8') ?> Ã‚Â· <?= htmlspecialchars($activeStay['pais'] ?? '-', ENT_QUOTES, 'UTF-8') ?></p>
                                        <p class="mt-3 text-xs text-slate-700 dark:text-slate-200">
                                ...
                                <?php foreach ($stayHistory as $stay) : ?>
                                    <article class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-5">
                                        ...
                                        <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100"><?= htmlspecialchars($stay['institucion'] ?? '-', ENT_QUOTES, 'UTF-8') ?> Ã‚Â· <?= htmlspecialchars($stay['pais'] ?? '-', ENT_QUOTES, 'UTF-8') ?></p>
                    ...
                    <!-- Cambio de contraseÃƒÂ±a -->
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 p-8 mt-6">
                        <h3 class="text-primary text-sm font-bold uppercase tracking-wider mb-4 flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm">key</span>
                            <span id="secPwd">Actualizar contraseÃƒÂ±a</span>
                        </h3>
                        <p id="secPwdDesc" class="text-sm text-slate-600 dark:text-slate-300 mb-4">Cambia tu contraseÃƒÂ±a de acceso. Debe tener al menos 6 caracteres.</p>
                        <form id="pwdInlineForm" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="flex flex-col gap-2">
                                <label id="lblPwdCurrent" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">ContraseÃƒÂ±a actual</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label id="lblPwdNew" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Nueva contraseÃƒÂ±a</label>
                            ...
                            <div class="md:col-span-3 flex items-center gap-3">
                                <button id="pwdSubmit" type="submit" class="h-11 px-5 rounded-lg border border-primary bg-white text-primary font-semibold hover:bg-primary hover:text-white transition-colors flex items-center gap-2">
                                    <span id="pwdSubmitText">Guardar nueva contraseÃƒÂ±a / Save new password</span>
    ...
    <script>
        ...
        function switchLanguage(lang) {
            const t = {
                es: {
                    ...
                    secPersonal: 'InformaciÃƒÂ³n Personal',
                    lblFoto: 'Foto',
                    lblNombre: 'Nombre',
                    lblApellidos: 'Apellidos',
                    lblDni: 'DNI/Pasaporte',
                    lblNacimiento: 'Fecha de Nacimiento',
                    lblTelefono: 'TelÃƒÂ©fono',
                    secAcademico: 'Origen AcadÃƒÂ©mico',
                    lblInstitucion: 'InstituciÃƒÂ³n',
                    lblPais: 'PaÃƒÂ­s',
                    secIncorporacion: 'Detalles de la IncorporaciÃƒÂ³n',
                    lblMotivo: 'Motivo',
                    lblInicio: 'Fecha de Inicio',
                    lblFin: 'Fecha de FinalizaciÃƒÂ³n',
                    lblGrupo: 'Grupo',
                    secHistorial: 'Historial de estancias',
                    secPwd: 'Actualizar contraseÃƒÂ±a',
                    secPwdDesc: 'Cambia tu contraseÃƒÂ±a de acceso. Debe tener al menos 6 caracteres.',
                    lblPwdCurrent: 'ContraseÃƒÂ±a actual',
                    lblPwdNew: 'Nueva contraseÃƒÂ±a',
                    lblPwdConfirm: 'Confirmar nueva',
                    pwdSubmitText: 'Guardar nueva contraseÃƒÂ±a',
                },
                en: {
                    headerTitle: 'Welcome',
                    greetText: 'Hello',
        ...
        // Toast reutilizable con estÃƒÂ©tica del panel (morado)
        const toastHost = (() => {
        ...
        (() => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    btnText.textContent = lang === 'en' ? 'Save new password' : 'Guardar nueva contraseÃƒÂ±a / Save new password';
                }
            });
        })();
    </script>
