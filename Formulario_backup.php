...
<input class="form-input rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 focus:ring-primary focus:border-primary h-12 text-slate-600 dark:text-slate-300" type="date" name="fecha_nacimiento"/>
</label>
<label class="flex flex-col gap-2">
...
<label class="flex flex-col gap-2">
<p class="text-slate-700 dark:text-slate-300 text-sm font-semibold">ContraseÃ±a / Password</p>
<input class="form-input rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 focus:ring-primary focus:border-primary placeholder:text-slate-400 dark:placeholder:text-slate-600 h-12" placeholder="*******" type="password" name="password" required/>
</label>
</div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function handleFile(file) {
            ...
            if (file.size > maxBytes) {
                showPhotoError("El archivo es demasiado grande. M�ximo 5 MB. / File is too large. Max 5 MB.");
                return;
            }
            clearPhotoError();
            selectedPhoto = file;
        ...
        photoRemoveButton.addEventListener('click', resetPreview);
        ...
        // Gestión de fechas y personal fijo
        ...
        // Envío del formulario a la API con validación visual
        ...
        form?.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (!res.ok || json.error) {
                    showPhotoError(json.error || 'Error al registrar. Int�ntelo de nuevo.');
                    return;
                }
                form.classList.add('ring-2', 'ring-primary/40');
                setTimeout(() => form.classList.remove('ring-2', 'ring-primary/40'), 1200);
                form.reset();
                toggleFechas();
                resetPreview();
                selectedPhoto = null;
